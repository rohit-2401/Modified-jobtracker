...
def index():
    ...
    if request.method == "POST":
        ...
        if not (company or position or company_link):
            return apology("must provide al fields", 400)

        db.execute("INSERT INTO applications (user, company, position, company_link, status, last_updated) VALUES(?,?,?,?,?,?)",
                   user_id, company, position, company_link, "Applied", (last_updated,))
        ...
        # get applications
        rows = db.execute(
            "SELECT id, company, position, company_link, status, last_updated from applications WHERE user = :user_id", user_id=user_id)
